...
parse_log(data_lan, list_net_conds[0], base_path=base_dir)
...
def generate_table_main_body(data_mat):
    ...
    for i, row in enumerate(data_mat):
        if len(row) != 3:
            raise ValueError("Each data row must contain exactly 3 values.")
...
def data_format_print_main_body(data: dict):
    data_mat = []
    for alg in list_algs:
        data_mat.append([])
        for executable in list_schemes:
            data_mat[-1].append(f"{data[alg][executable]['duration'][0]} ({data[alg][executable]['communication'][0]})")            
