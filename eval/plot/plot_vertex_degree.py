...
list_num_parts = [6]
list_scales = [16] # 2 ^ n
list_average_degrees = [3, 4, 5, 6, 7, 8, 9, 10]
...
for executable in list_schemes:
    ...
    scale = list_scales[0]
    inter_ratio = list_inter_ratios[0]
    num_parts = list_num_parts[0]
    for average_degree in list_average_degrees:
        for alg in list_algs:            
            # Initialize data structure for this algorithm if not already done
            if alg not in data:
                data[alg] = {}
            if executable not in data[alg]:
                data[alg][executable] = {'average_degree': [], 'duration': [], 'communication': []}
            
            # Read the log file
            with open(os.path.join(base_dir, "executable_" + str(executable), "net_cond_" + str(net_cond[0]) + "_" + str(net_cond[1]), "num_parts_" + str(num_parts), "scale_" + str(scale), "avgDegree_" + str(average_degree), "interRatio_" + str(inter_ratio), "alg_" + str(alg), "iters_" + str(iterations), "efficiency_0.log"), 'r') as f:
                lines = f.readlines()
                duration = 0.0
                communication = 0.0
                for line in lines:
                ...
                data[alg][executable]['average_degree'].append(average_degree)
...
for alg in list_algs:
    ...
    for executable in data[alg]:
        axes[row, 0].plot(data[alg][executable]['average_degree'], data[alg][executable]['duration'], markerList[executable], linewidth=1, ls='-', ms=8, color=colors[executable], label=f'{list_scheme_formal_names[executable]}')
        axes[row, 1].plot(data[alg][executable]['average_degree'], [x/1024 for x in data[alg][executable]['communication']], markerList[executable], linewidth=1, ls='-', ms=8, color=colors[executable], label=f'{list_scheme_formal_names[executable]}')
